...
### **Modified Files (1)**
- ✅ `.github/workflows/security.yml` - **6 CHANGES APPLIED**
  1. Added `fetch-depth: 0` to checkout for full history download (line 24)
  2. Added `GITLEAKS_LOG_OPTS: "--all"` to actually scan git history (line 76)
  3. Removed duplicate `secret-detection` job
  4. Kept Gitleaks in main `security` job (line 68-77)
  5. Removed placeholder "banned dependencies" check
  6. Improved GPL license check to target AGPL specifically
