...
import type { ExtendedPerformance, ExtendedNavigator } from '../../types/browser';

export type LogLevel = 'debug' | 'info' | 'warn' | 'error';
export type LogCategory = 'performance' | 'optimization' | 'cache' | 'preloading' | 'analysis' | 'monitoring';
...
class StructuredPerformanceLogger {
  ...
  constructor(config: Partial<LoggingConfig> = {}) {
    this.config = {
      ...
      categories: ['performance', 'optimization', 'cache', 'preloading', 'analysis', 'monitoring'],
  ...
  debug(category: LogCategory, message: string, context?: Record<string, any>, performanceData?: PerformanceLogData): void {
    this.log('debug', category, message, context, undefined, performanceData);
  }

  info(category: LogCategory, message: string, context?: Record<string, any>, performanceData?: PerformanceLogData): void {
    this.log('info', category, message, context, undefined, performanceData);
  }

  warn(category: LogCategory, message: string, context?: Record<string, any>, performanceData?: PerformanceLogData): void {
    this.log('warn', category, message, context, undefined, performanceData);
  }

  error(category: LogCategory, message: string, context?: Record<string, any>, error?: Error, performanceData?: PerformanceLogData): void {
    this.log('error', category, message, context, error, performanceData);
  }

  // Specialized performance logging methods
  logPerformanceMetric(operation: string, duration: number, context?: Record<string, any>): void {
  ...
  logOptimization(type: string, impact: number, context?: Record<string, any>): void {
  ...
  logCacheOperation(operation: string, hit: boolean, url?: string, context?: Record<string, any>): void {
    this.debug('cache', `Cache ${operation}: ${hit ? 'HIT' : 'MISS'}`, context, {
      operation,
      cacheHit: hit,
      resourceUrl: url
    });
  ...
  logPreloadOperation(url: string, success: boolean, context?: Record<string, any>): void {
    this.debug('preloading', `Preload ${success ? 'succeeded' : 'failed'}: ${url}`, context, {
      operation: 'preload',
      resourceUrl: url
    });
  }

  logBundleAnalysis(bundleName: string, size: number, context?: Record<string, any>): void {
  ...
  private trimBuffer(): void {
    if (this.logBuffer.length > this.config.maxLocalEntries) {
      this.logBuffer.splice(0, this.logBuffer.length - this.config.maxLocalEntries);
    }
  }
  ...
  updateConfig(updates: Partial<LoggingConfig>): void {
    ...
    this.info('monitoring', 'Logging configuration updated', { newConfig: updates });
  }
  ...
  generateReport(): {
    ...
    const categories: Record<LogCategory, number> = {
      performance: 0, optimization: 0, cache: 0, 
      preloading: 0, analysis: 0, monitoring: 0
    };
