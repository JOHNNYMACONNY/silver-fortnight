...
jest.mock('../../ui/ProfileImageWithUser', () => {
  return function MockProfileImageWithUser({ user, size }: { user: any; size?: string }) {
    return (
...
import { RoleState, ApplicationStatus, CollaborationRoleData } from '../../../types/collaboration';
...
const mockRoleApplications = roleApplications as jest.Mocked<typeof roleApplications>;
const mockRoleCompletions = roleCompletions as jest.Mocked<typeof roleCompletions>;
const mockRoleAbandonment = roleAbandonment as jest.Mocked<typeof roleAbandonment>;
...
describe('CollaborationRolesSection Integration Tests', () => {
  ...
  describe('Role Application Flow', () => {
    it('should handle complete role application workflow', async () => {
      ...
      expect(screen.getByLabelText(/why are you a good fit for this role/i)).toBeInTheDocument();
    ...
    it('should prevent application when user is not logged in', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('You must be logged in to apply for a role')).toBeInTheDocument();
      });
    ...
    it('should handle application errors gracefully', async () => {
      ...
      const messageTextarea = screen.getByLabelText(/why are you a good fit for this role/i);
  ...
  describe('Role Completion Flow', () => {
    it('should handle role completion request workflow', async () => {
      ...
      const filledRolesSection = screen.getByText('Filled Roles').closest('div');
      const roleCard = within(filledRolesSection!).getByText('UI/UX Designer').closest('[class*="backdrop-blur"]') as HTMLElement;
      expect(roleCard).toBeInTheDocument();

      // Click on "Request Role Completion" button
      const requestButton = within(roleCard!).getByText('Request Role Completion');
      ...
      await user.type(notesTextarea, 'All wireframes and designs have been completed and approved');
      ...
      await waitFor(() => {
        expect(mockRoleCompletions.requestRoleCompletion).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            notes: 'All wireframes and designs have been completed and approved'
          })
        );
      });
  ...
  describe('Status Tracking and Visualization', () => {
    ...
    it('should calculate and display correct progress metrics', () => {
      ...
      const openRoles = mockRoles.filter(role => role.status === RoleState.OPEN);
      const filledRoles = mockRoles.filter(role => 
        role.status === RoleState.FILLED || role.status === RoleState.COMPLETED
      );
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should prevent actions for roles user cannot interact with', () => {
      ...
      const backendRole = screen.getByText('Backend Developer').closest('[class*="backdrop-blur"]') as HTMLElement;
      expect(within(backendRole!).queryByText('Apply')).not.toBeInTheDocument();

      // Should only show apply button for open roles
      const frontendRole = screen.getByText('Frontend Developer').closest('[class*="backdrop-blur"]') as HTMLElement;
  ...
  describe('Accessibility and User Experience', () => {
    ...
    it('should provide clear loading states during operations', async () => {
      ...
      const loadingButton = screen.getByRole('button', { name: /submit application/i });
