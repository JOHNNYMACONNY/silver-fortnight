// src/components/features/SocialFeaturesWithRealtime.tsx
...
interface SocialFeaturesWithRealtimeProps {
  userId: string;
  userName: string;
  userAvatar?: string;
...
export const SocialFeaturesWithRealtime: React.FC<SocialFeaturesWithRealtimeProps> = ({
  userId,
  userName,
  userAvatar,
  ...
  // Real-time social stats (if enabled)
  const { 
    stats: realtimeStats, 
    ...
    userId,
    enabled: enableRealtime && showStats,
    onUpdate: (stats) => {
      console.log('Real-time social stats updated:', stats);
    }
  });

  // Real-time follow count (if enabled)
  const { 
    followersCount: realtimeFollowersCount, 
    ...
    enabled: enableRealtime && showStats,
    onUpdate: (count) => {
      console.log('Real-time followers count updated:', count);
    }
  });

  // Fallback to regular hooks if real-time is disabled
  const { 
    data: fallbackStats, 
    loading: fallbackStatsLoading, 
  ...
  const socialStats = enableRealtime ? realtimeStats : fallbackStats;
  const statsLoading = enableRealtime ? realtimeStatsLoading : fallbackStatsLoading;
  const followersCount = enableRealtime ? realtimeFollowersCount : (socialStats?.followersCount || 0);
  ...
  const getConnectionStatus = () => {
    if (!enableRealtime) {
      return { connected: false, text: 'Static', icon: WifiOff, color: 'text-gray-500' };
    }
      ...
      {showConnectionStatus && enableRealtime && (
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
          ...
          <div className="text-xs text-gray-500">
            {enableRealtime && statsLastUpdate && `Updated ${new Date(statsLastUpdate).toLocaleTimeString()}`}
          </div>
        </div>
      )}
...
const UserSocialStats: React.FC<UserSocialStatsProps> = ({
  ...
  const { 
    ...
    onUpdate: (stats) => {
      console.log('Real-time social stats updated in UserSocialStats:', stats);
    }
  });
  ...
  const { 
    ...
    enabled: enableRealtime,
    onUpdate: (count) => {
      console.log('Real-time followers count updated in UserSocialStats:', count);
    }
  });
