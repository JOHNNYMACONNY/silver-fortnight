...
jest.mock("../../../../services/challenges", () => ({
  ...
  getWeeklyChallenges: jest.fn().mockResolvedValue({
    success: true,
    challenges: [
      {
        id: "w1",
...
describe("ChallengeCalendar strip", () => {
  ...
  it.skip("renders daily/weekly links and a View all link", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Challenge Calendar/i)).toBeInTheDocument()
    );

    // Use getByRole to target the specific link elements (not the sr-only announcement)
    expect(screen.getByRole("link", { name: /Daily A/i })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Weekly A/i })).toBeInTheDocument();
