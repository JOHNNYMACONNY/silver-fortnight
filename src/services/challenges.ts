...
const challengeEventListeners: Array<(notification: ChallengeNotification) => void> = [];

/**
 * Set callback for real-time challenge notifications
 */
export const setChallengeNotificationCallback = (callback: (notification: ChallengeNotification) => void) => {
 ...
 */
export const addChallengeEventListener = (listener: (notification: ChallengeNotification) => void): void => {
 ...
 */
export const removeChallengeEventListener = (listener: (notification: ChallengeNotification) => void): void => {
 ...
 */
export const createChallenge = async (challengeData: Partial<Challenge>): Promise<ChallengeResponse> => {
  try {
    ...
    const challenge: Challenge = {
      ...
      rewards: challengeData.rewards || { xp: XP_VALUES.CHALLENGE_COMPLETION_BEGINNER },
 ...
 */
export const getChallenge = async (challengeId: string): Promise<ChallengeResponse> => {
  try {
 ...
 */
export const getChallenges = async (filters: ChallengeFilters = {}): Promise<ChallengeListResponse> => {
  try {
 ...
 * Join a challenge
 */
export const joinChallenge = async (challengeId: string, userId: string): Promise<ChallengeResponse> => {
  try {
...
export const updateChallengeProgress = async (
  ...
  try {
    const result = await runTransaction(getSyncFirebaseDb(), async (transaction) => {
      ...
      const newProgress = Math.min(userChallenge.progress + progressIncrement, userChallenge.maxProgress);
      const progressPercentage = (newProgress / userChallenge.maxProgress) * 100;
      ...
      const updatedUserChallenge: UserChallenge = {
        ...
        lastActivityAt: Timestamp.now(),
        status: isCompleted ? UserChallengeStatus.COMPLETED : UserChallengeStatus.ACTIVE,
        completedAt: isCompleted ? Timestamp.now() : undefined,
        completionTimeMinutes: isCompleted && userChallenge.startedAt
          ? Math.round((Timestamp.now().toMillis() - userChallenge.startedAt.toMillis()) / 60000)
          : undefined
      };

      transaction.set(userChallengeRef, updatedUserChallenge);

      return { userChallenge: updatedUserChallenge, progressPercentage, isCompleted };
    });
    ...
    if (result.isCompleted) {
      await handleChallengeCompletion(userId, challengeId, result.userChallenge);
    } else {
...
const handleChallengeCompletion = async (
  ...
  try {
    ...
    if (userChallenge.completionTimeMinutes && challenge.timeEstimate) {
      ...
      if (estimatedMinutes && userChallenge.completionTimeMinutes < estimatedMinutes * 0.75) {
...
export const submitToChallenge = async (
  ...
  try {
    const submissionRef = doc(collection(getSyncFirebaseDb(), 'challengeSubmissions'));
    const challengeSubmission: ChallengeSubmission = {
      ...
      embeddedEvidence: (submission.embeddedEvidence as EmbeddedEvidence[]) || undefined,
    ...
    await updateDoc(doc(getSyncFirebaseDb(), 'userChallenges', `${userId}_${challengeId}`), {
 ...
 * Abandon a challenge
 */
export const abandonChallenge = async (userId: string, challengeId: string): Promise<ChallengeResponse> => {
  try {
    const userChallengeRef = doc(getSyncFirebaseDb(), 'userChallenges', `${userId}_${challengeId}`);
 ...
 */
export const leaveChallenge = async (userId: string, challengeId: string): Promise<ChallengeResponse> => {
 ...
 */
export const startChallenge = async (userId: string, challengeId: string): Promise<{ 
  success: boolean; 
  userChallenge?: UserChallenge; 
  error?: string 
}> => {
 ...
 */
export const getChallengeStats = async (userId: string): Promise<{
  ...
  try {
    ...
    const completionRate = totalChallenges > 0 ? (stats.totalCompleted / totalChallenges) * 100 : 0;
 ...
 */
export const completeChallenge = async (userChallengeId: string): Promise<{
  success: boolean;
  userChallenge?: UserChallenge;
  error?: string;
}> => {
  try {
    const userChallengeRef = doc(getSyncFirebaseDb(), 'userChallenges', userChallengeId);
    const userChallengeSnap = await getDoc(userChallengeRef);
    ...
    const updatedData = {
      ...
      lastActivityAt: Timestamp.now(),
      completionTimeMinutes: Math.floor((Date.now() - userChallenge.startedAt.toMillis()) / (1000 * 60))
    };
 ...
 */
export const getUserActiveChallenges = async (userId: string): Promise<ChallengeListResponse> => {
 ...
 */
export const getUserCompletedChallenges = async (userId: string): Promise<ChallengeListResponse> => {
...
export const getUserChallengeStats = async (userId: string): Promise<{
  ...
  try {
    ...
    const userChallenges = snapshot.docs.map(doc => doc.data() as UserChallenge);

    const completed = userChallenges.filter(uc => uc.status === UserChallengeStatus.COMPLETED);
    const active = userChallenges.filter(uc => uc.status === UserChallengeStatus.ACTIVE);

    const totalCompletionTime = completed.reduce((sum, uc) => 
      sum + (uc.completionTimeMinutes || 0), 0
    );
