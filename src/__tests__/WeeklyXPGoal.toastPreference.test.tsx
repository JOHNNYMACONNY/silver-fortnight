...
jest.mock("../services/gamification", () => ({
  getUserXPHistory: jest.fn(async () => {
    const now = new Date();
    const mkTs = (d: Date) => ({ toDate: () => d } as unknown);
    return {
      success: true,
      data: [
        {
          amount: 200,
          createdAt: mkTs(new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000)),
        },
        {
          amount: 200,
          createdAt: mkTs(new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000)),
        },
        {
          amount: 200,
          createdAt: mkTs(new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000)),
        },
      ],
    };
  }),
...
jest.mock("../contexts/GamificationNotificationContext", () => ({
  useGamificationNotifications: () => ({
    preferences: { weeklyGoalMetToasts: false },
  }),
}));
...
describe("WeeklyXPGoal - weeklyGoalMetToasts preference", () => {
  ...
  it("suppresses toast when weeklyGoalMetToasts is false but still tracks analytics", async () => {
    render(<WeeklyXPGoal userId={userId} target={600} />);
    await act(async () => {});

    // Analytics fired once, toast not called
    expect(mockTrack).toHaveBeenCalledWith(
      "weekly_goal_met",
      expect.objectContaining({ target: 600 })
    );
