...
export function generateId(): string {
  // @ts-ignore - globalThis.crypto may be undefined in some runtimes
...
export type IntegrityRepairAction =
  ...
  | { action: 'archived_duplicate'; details: { originalId: string; archivedId: string } }
  | { action: 'deduplicated_active'; details: { keptId: string; removedId: string } };
...
export type IntegrityAnomaly =
  | { type: 'duplicate_active_content'; details: { content: string; ids: string[]; count: number } }
  | { type: 'order_gap'; details: { expectedOrder: number; actualOrder: number; id: string } }
  | { type: 'invalid_state'; details: { id: string; issue: string; status: TodoStatus } }
...
export function createTodo(content: string, order: number, tags?: string[] | null): Todo {
...
export function computeMetrics(todos: readonly Todo[]): Metrics {
  ...
  return { total, pending, inProgress, completed, archived, active, completionRatio };
}

export function createLogEvent(event: TodoEvent, correlationId?: string): StructuredLogEvent {
  return {
