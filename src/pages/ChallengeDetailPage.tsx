...
export const ChallengeDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const unlocked = progress.success
          ? progress.data?.unlockedTiers || []
          : [];
        if (type === "TRADE" && !unlocked.includes("TRADE")) {
          setIsLocked(true);
          setLockReason(
            "Complete 3 Solo challenges and reach skill level 2 to unlock Trade challenges."
          );
        } else if (
          type === "COLLABORATION" &&
          ...
          setIsLocked(true);
          setLockReason(
            "Complete 5 Trade challenges and reach skill level 3 to unlock Collaboration challenges."
          );
        } else {
          setIsLocked(false);
          setLockReason("");
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <article
        className="glassmorphic overflow-hidden transition-all duration-200"
        role="article"
        ...
        <header className="px-6 py-5 border-b border-glass flex justify-between items-center transition-colors duration-200">
            ...
            <div className="flex items-center mt-2 space-x-4">
              {
                ...
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getDifficultyClass(
              ...
              {(() => {
                const remaining = getTimeRemaining(
                  (challenge as any).endDate || (challenge as any).deadline
                );
          ...
          {normalizeStatus((challenge as any).status) === "OPEN" && (
            <button
              ...
              aria-label={
                isParticipating
                  ? "Currently participating in challenge"
                  : "Join this challenge"
              }
            >
        ...
        <div className="p-6">
          ...
          <section className="mb-8" aria-labelledby="challenge-details">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              {((challenge as any).deadline?.toDate ||
                (challenge as any).endDate?.toDate) && (
                <div className="flex items-center text-sm text-muted-foreground">
              ...
              {(challenge as any).createdAt &&
                (challenge as any).createdAt.toDate && (
                  <div className="flex items-center text-sm text-muted-foreground">
                    <Calendar className="mr-2 h-4 w-4 text-muted-foreground" />
                    <span>
                      Posted:{" "}
                      {formatDate((challenge as any).createdAt.toDate())}
                    </span>
                  </div>
                )}

              {((challenge as any).participants ||
                typeof (challenge as any).participantCount === "number") && (
                <div className="flex items-center text-sm text-muted-foreground">
          ...
          <section className="mb-8" aria-labelledby="rewards-section">
            ...
            >
              ...
              <Tooltip
                content={
                  <div>
                    Base XP is guaranteed on completion. Bonus XP depends on
          ...
          {isLocked && (
            <section
              className="mb-8 glassmorphic p-4 rounded-lg border-glass bg-amber-50/50 dark:bg-amber-900/20 backdrop-blur-sm"
              aria-labelledby="unlock-criteria"
            >
              <h3
                id="unlock-criteria"
            ...
            <section className="mb-8" aria-labelledby="recent-submissions">
              ...
              <div className="space-y-6">
                {submissions.slice(0, 5).map((submission) => (
                  <div
                    key={submission.id}
                    className="glassmorphic p-4 rounded-lg border-glass bg-card/20 backdrop-blur-sm"
                  >
          ...
          onClose={() => {
            ...
            addToast(
              ...
              `Challenge completed! You earned ${
                completionRewards.xp + completionRewards.bonusXP
              } XP!`
            );
          }}
