...
import { Calendar, Users, CheckCircle2, AlertCircle, MessageSquare, Edit } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="card card-form">
        <div className="p-6">
          ...
          <div className="flex gap-4">
            ...
            {(isCreator || (isAdmin && adminModeEnabled)) && (
              <button
                type="button"
                onClick={() => {
                  console.log('Project Details: Edit button clicked');
                  navigate(`/projects/${id}/edit`);
                }}
                className="btn-secondary btn-full py-3 flex items-center justify-center gap-3 relative z-50 hover:bg-accent-sage active:bg-accent-sage focus:ring-2 focus:ring-offset-2 focus:ring-accent-sage text-lg font-semibold"
              >
                <Edit className="h-6 w-6 text-accent-sage" />
                Edit Project
              </button>
            )}
          </div>
      ...
      {showStatusModal && (
        <div className="fixed inset-0 flex items-center justify-center p-4 z-50">
          ...
          <div className="card card-form p-6 max-w-md w-full relative isolation-isolate">
            <h3 className="text-lg font-semibold mb-4">Update Project Status</h3>
            <div className="space-y-3 relative z-[70]">
              {project.status === 'recruiting' && (
                <button
                  onClick={() => handleUpdateStatus('in-progress')}
                  disabled={!allRolesFilled || updatingStatus}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded bg-white hover:bg-gray-50 text-gray-900 border border-accent-clay transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              {project.status !== 'completed' && (
                <button
                  ...
                  disabled={updatingStatus}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded bg-white hover:bg-gray-50 text-gray-900 border border-accent-ochre transition-colors"
                >
                  <CheckCircle2 className="h-5 w-5" />
                  Mark as Completed
                </button>
              ...
              {project.status !== 'recruiting' && (
                <button
                  ...
                  disabled={updatingStatus}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded bg-white hover:bg-gray-50 text-gray-900 border border-accent-sage transition-colors"
                >
                  Reopen Recruitment
                </button>
              )}
              <button
                ...
                className="w-full px-4 py-2 text-gray-600 hover:bg-gray-50 rounded border border-gray-200 transition-colors mt-2"
              >
                Cancel
              </button>
            </div>
