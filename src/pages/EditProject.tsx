...
export function EditProject() {
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'recruiting' as Collaboration['status']
  });

  // Fetch project data and check permissions
  useEffect(() => {
    const fetchProjectAndCheckPermissions = async () => {
      if (!id || !user) {
        console.log('Edit Project: Missing id or user', { id, userId: user?.uid });
        return;
      }
      try {
        ...
        if (!projectDoc.exists()) {
          console.log('Edit Project: Project not found', { id });
          console.log('Edit Project: Project not found, delaying navigation');
          setError('Project not found');
          setTimeout(() => navigate('/'), 2000);
          return;
        }

        const projectData = projectDoc.data() as Collaboration;
        
        // Check if user has permission to edit
        const isCreator = projectData.creatorId === user.uid;
        ...
        if (!hasPermission) {
          console.log('Edit Project: Permission denied');
          console.log('Edit Project: Permission denied, delaying navigation');
        ...
        console.log('Edit Project: Error occurred, delaying navigation');
        setError('Failed to load project');
        setTimeout(() => navigate('/'), 2000);
      }
    }
    fetchProjectAndCheckPermissions();
  ...
  const handleAddRole = () => {
    ...
    setFormData(prev => ({
      ...prev,
      roles: [...prev.roles, { title: title.trim(), skills, filled: false }],
      currentRole: {
        title: '',
        skill: '',
        skills: []
  ...
  const handleToggleRoleFilled = (index: number) => {
    setFormData(prev => ({
      ...prev,
      roles: prev.roles.map((role, i) => 
        i === index ? { ...role, filled: !role.filled } : role
      )
    }));
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
      <div className="bg-earth-800 border border-earth-700 rounded-lg overflow-hidden relative">
        <div className="p-6 relative">
          <h1 className="text-2xl font-display font-bold text-text-primary mb-6">Edit Project</h1>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6 relative z-10">
              ...
              <select
                ...
                onChange={(e) => setFormData(prev => ({ 
                  ...prev, 
                  status: e.target.value as Collaboration['status']
                }))}
                className="w-full p-2 bg-earth-50 border border-earth-700 rounded-lg text-text-primary relative z-10
                          focus:border-accent-clay focus:ring-1 focus:ring-accent-clay/30 transition-all duration-300"
              >
                    ...
                    <div key={index} className="flex items-center justify-between p-3 bg-earth-700/20 
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => handleToggleRoleFilled(index)}
                            className={`px-2 py-0.5 text-xs rounded-full ${
                              role.filled
                                ? 'bg-accent-sage/20 text-accent-sage'
                                : 'bg-accent-clay/20 text-accent-clay'
                            }`}
                          >
                      ...
                      <button
                        ...
                        className="text-accent-rust hover:text-accent-rust/80 transition-colors duration-300 ml-4"
                      >
                        <X className="h-5 w-5" />
                      </button>
                    </div>
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 bg-earth-700 text-text-secondary py-3 rounded-lg hover:bg-earth-600 
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex-1 bg-accent-moss text-earth-50 py-3 rounded-lg hover:bg-accent-moss/90 
                          disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-300"
              >
